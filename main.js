...
let Tod = `Welcome To ${metadata.subject} ${num.split("@")[0]}`
                if (anu.action == 'add') {
                    let message = await prepareWAMessageMedia({ image: {url: ppuser }}, { upload: kirbotz.waUploadToServer })
                const template = generateWAMessageFromContent(anu.id, proto.Message.fromObject({
                    templateMessage: {
                        hydratedTemplate: {
                            imageMessage: message.imageMessage,
                            hydratedContentText: Tod,
                            hydratedFooterText: 'Welcome',
                            hydratedButtons: [{
                            	urlButton: {
                                displayText: '🧸YOUTUBE',
                                    url: 'https://youtu.be/Sk6Mt9Hyytc'
                                }
                            }, {
                                quickReplyButton: {
                                    displayText: 'OWNER',
                ...
                kirbotz.relayMessage(anu.id, template.message, { messageId: template.key.id })
